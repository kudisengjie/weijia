...
async function login(account){await page.goto('http://127.0.0.1:8769/');await page.locator('#login-page').waitFor({state:'visible'});await page.getByRole('textbox',{name:'账号',exact:true}).fill(account);await page.locator('#login-password').fill('test-password');await page.getByRole('button',{name:'登录工作台'}).click();await page.locator('.geo-shell').waitFor({state:'visible'});await page.getByRole('button',{name:'设置',exact:true}).click();await page.locator('#account-service').waitFor({state:'visible'});}
try{
  await login(owner);
  if(process.argv.includes('--layout-only')){
    await page.locator('#tenant-admin').scrollIntoViewIfNeeded();
    assert.ok(await page.locator('#managed-user').evaluate(el=>el.getBoundingClientRect().height)>=44);
  ...
  assert.equal(await page.locator('#account-service .wechat-contact').isVisible(),true);
