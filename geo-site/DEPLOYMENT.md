...
5. 如果仍提示 `STORAGE_UNAVAILABLE`，打开此次部署的函数日志，查找 `blob_init_failed` 及其脱敏的 `name`、`code`。页面错误来自存储初始化阶段，不代表账号或密码错误。Blob SDK 已按官方示例由函数入口静态导入，不能重新加入 `externalNodeModules`，否则平台无法向 bundle 注入部署凭据。如控制台没有 Blob 入口，先确认当前账号/项目的功能支持，不要盲目购买 CloudBase。
