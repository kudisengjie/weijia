...
本说明对应 SaaS 发布候选版本（schemaVersion=3）。是否推送成功以 GitHub master 的提交号为准，是否部署以 EdgeOne 的部署记录为准。所有密码、API Key、主密钥和数据库连接串都只能放在服务端环境或密钥管理中，不得进入 Git、截图、日志或本文档。
...
启动迁移是幂等的，当前 schemaVersion 为 3；新增表和列由服务端初始化，不要手动删表修复环境。
...
1. 沿用原有 GEO_MASTER_KEY。服务端首次初始化会在一个事务中加密已有批次、清空明文副本并记录 v3；重复初始化不会重新加密或扣分。账号、任务序号、文章文件和积分流水保留。
2. 部署前先暂停批次并等待当前调用完成，再发布前端/API；如已有 Worker，先正常停止，再用同一提交启动。不要混跑旧版明文写入程序。数据库约束会拒绝旧程序写回明文；不能直接回退到 v1/v2 代码。
...
3. 线上检查 /api/health 返回 schemaVersion=3、ready=true，再检查真实登录、账户隔离、余额、IMA 配置状态和文章下载。
