...
export function initializeConsole({changeView}){
  ...
  const materials=el('div'),materialSummary=el('section',undefined,'runtime-panel');materialSummary.id='workspace-materials-summary';materialSummary.hidden=true;
  workspace.insertBefore(materials,uploads);materials.append(uploads,materialSummary);
  ...
  function setBatch(batch){
    ...
    uploads.hidden=Boolean(batch);materialSummary.hidden=!batch;materialSummary.replaceChildren();
  ...
  function reset(){setOwner(false);setBatch(null);byId('overview-list').replaceChildren();for(const id of ['overview-total','overview-active','overview-articles'])byId(id).textContent='—';renderArticles(null);personal.select('models');management.select('members');detailTab('materials');changeView('overview');}
  setOwner(false);setBatch(null);changeView('overview');
  return {setOwner,setBatch,detailTab,renderOverview,renderArticles,reset};
}
