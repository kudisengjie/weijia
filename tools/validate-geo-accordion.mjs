...
for (const path of allPages) {
  const content = read(path);
  assert(content.includes('data-i18n="nav.cases">GEO策略</a>'), `${path} should use the current strategy navigation label`);
...
assert(blog.includes('2026-07-26T00:00:00+08:00'), 'blog modified time should match the current source update');
