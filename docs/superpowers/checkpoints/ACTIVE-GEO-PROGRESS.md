...
后续必须完成：管理界面选择子账号、续期和流水；运行页取消/退款展示/全局模型锁；订阅调整目标校验与审计；旧会话初始化；Worker 到期后的退款/暂停语义、生产部署文档和真实浏览器联调。新 Worker CLI 在本机可运行，但尚未部署到独立主机；只有 EdgeOne 并不等于已有后台常驻进程。禁止直接把未完成版本推送到线上。
